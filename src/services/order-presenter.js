...
export function presentOrder(order){
  const next=kitchen[order.status],step=order.provider==="mock"?simulation[order.status]:null;
  return {...order,statusLabel:labels[order.status]||order.status,nextStatus:next?.[0],nextLabel:next?.[1],simulationStatus:step?.[0],simulationLabel:step?.[1],canCancel:["new","accepted","preparing","ready"].includes(order.status),canQuote:order.status==="ready",finished:["delivered","cancelled"].includes(order.status)};
}
