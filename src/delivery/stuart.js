function required(value,name){
  if(!value)throw new Error(`Falta configuración Stuart: ${name}`);
  return value;
}
...
async function request(config,path,options={}){
  ...
  if(!response.ok)throw new Error(`Stuart ${response.status}: ${typeof data==="string"?data:JSON.stringify(data)}`);
  return data;
}
...
export function createStuartDelivery(config){
  return {
    name:"stuart",
    async testConnection(){await getToken(config);return {provider:"stuart",ok:true}},
    async quote(order){
      const settings=config.settings||{},token=await getToken(config);
      const data=await request(config,required(settings.quotePath,"quotePath"),{method:"POST",headers:headers(token),body:JSON.stringify({accountId:settings.accountId,pickup:{name:settings.pickupName,phone:settings.pickupPhone,address:settings.pickupAddress,latitude:settings.pickupLat?Number(settings.pickupLat):undefined,longitude:settings.pickupLng?Number(settings.pickupLng):undefined},dropoff:{address:order.delivery_formatted_address || order.delivery_address,latitude:Number(order.delivery_latitude),longitude:Number(order.delivery_longitude),name:order.customer_name,phone:order.customer_phone}})});
      return {provider:"stuart",quoteId:data.quoteId||data.id,feeCents:Number(data.feeCents ?? data.fee ?? 0),etaMinutes:Number(data.etaMinutes ?? data.durationMinutes ?? 0)||null,expiresAt:data.expiresAt?Date.parse(data.expiresAt):null,raw:data};
    },
    async create(order,quote){
      const settings=config.settings||{},token=await getToken(config);
      const data=await request(config,required(settings.createPath,"createPath"),{method:"POST",headers:headers(token),body:JSON.stringify({accountId:settings.accountId,quoteId:quote.quoteId,pickup:{name:settings.pickupName,phone:settings.pickupPhone,address:settings.pickupAddress},dropoff:{address:order.delivery_formatted_address || order.delivery_address,latitude:Number(order.delivery_latitude),longitude:Number(order.delivery_longitude),name:order.customer_name,phone:order.customer_phone,apartment:order.delivery_apartment,instructions:order.delivery_notes},items:order.items.map(item=>({name:item.product_name,quantity:item.quantity}))})});
      return {provider:"stuart",providerOrderId:data.deliveryId||data.orderId||data.id,providerStatus:data.status||"created",trackingUrl:data.trackingUrl||data.tracking_url,raw:data};
    }
  };
}
